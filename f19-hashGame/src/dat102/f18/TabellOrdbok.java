...
public class TabellOrdbok<K, V> implements OrdbokInterface<K, V> {
    ...
    public TabellOrdbok(int startKapasitet) {
        startKapasitet = sjekkKapasitet(startKapasitet);
        // "Bedre" alternativ:
        // startKapasitet = Math.max(startKapasitet, STANDARD_KAPASITET);

        this.antallElement = 0; // Gjøre dette inline ved deklarasjon?
        this.tabellStorrelse = finnNestePrimtall(startKapasitet);

        @SuppressWarnings("unchecked")
        TabellElement<K, V>[] temp = (TabellElement<K, V>[]) new TabellElement[tabellStorrelse];
        this.hashTabell = temp;
    }
    ...
    public V fjern(K nokkel) {
        ...
        if (index != -1) {
            // NB! Elementet vil fremdeles være der, men markert som fjerna
    ...
    private void utvidTabell() {
        ...
        TabellElement<K, V>[] gml = hashTabell;
        int nyStorrelse = finnNestePrimtall(2 * gml.length);

        @SuppressWarnings("unchecked")
        TabellElement<K, V>[] temp = (TabellElement<K, V>[]) new TabellElement[tabellStorrelse];
        hashTabell = temp;
        tabellStorrelse = nyStorrelse;
        antallElement = 0;

        // Plasserer elementa på nytt med ny tabellstørrelse
        for (int i = 0; i < gml.length; i++) {
            if(gml[i] != null && gml[i].erMed()) {
                leggTil(gml[i].getKey(), gml[i].getVerdi());
            }
        }
    }
